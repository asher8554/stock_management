...
assert.equal((await worker.fetch(new Request("https://api/v1/intraday", { method: "POST", headers: { authorization: "Bearer ingest" }, body: "{}" }), env)).status, 404);
assert.equal((await worker.fetch(new Request("https://api/v1/intraday?symbol=237350", { headers: { authorization: `Bearer ${session}` } }), env)).status, 404);
