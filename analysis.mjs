...
export const purchaseMarkers = (bars, purchases) => (Array.isArray(purchases) ? purchases : []).map((purchase) => {
  ...
  return index >= 0 && /^\d{8}$/.test(date) && Number.isFinite(number(purchase.price)) ? { ...purchase, side: purchase.side || "buy", date, index } : null;
}).filter(Boolean);
...
function renderChart(host, item, unit, range, zoom, indicators, syncCount) {
  ...
  // 모바일은 시간축이 화면에 맞게 축소되도록 논리 폭을 고정하고 SVG를 100%로 스케일한다.
  const mobileLogicalWidth = 860;
  const width = isMobileView ? mobileLogicalWidth : chartWidth(bars.length, effectiveZoom);
