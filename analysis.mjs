...
function renderChart(host, item, range) {
  const allBars = normalizeBars(item.bars);
  const start = Math.max(0, allBars.length - (ANALYSIS_RANGES[range] ?? Infinity));
  const bars = barsForRange(allBars, range);
...
async function init() {
  ...
  let savedRange = "";
  try { savedRange = localStorage.getItem(ANALYSIS_RANGE_STORAGE_KEY) || ""; } catch { /* Browser storage can be unavailable. */ }
  let selectedRange = Object.hasOwn(ANALYSIS_RANGES, savedRange) ? savedRange : "전체";
  const render = () => { holdings.replaceChildren(...items.map((item) => { const button = document.createElement("button"); button.type = "button"; button.className = item === selected ? "selected" : ""; button.textContent = `${item.name} (${item.symbol})`; button.addEventListener("click", () => { selected = item; localStorage.setItem(ANALYSIS_STORAGE_KEY, item.symbol); render(); }); return button; })); ranges.replaceChildren(...Object.keys(ANALYSIS_RANGES).map((range) => { const button = document.createElement("button"); button.type = "button"; button.className = range === selectedRange ? "selected" : ""; button.textContent = range; button.addEventListener("click", () => { selectedRange = range; localStorage.setItem(ANALYSIS_RANGE_STORAGE_KEY, range); render(); }); return button; })); renderChart(host, selected, selectedRange); };
  render();
}
