...
async function init() {
  ...
  host.addEventListener("pointermove", (event) => { const crosshair = host.querySelector(".chart-crosshair"); if (!crosshair) return; const index = chartHoverIndex(host.querySelectorAll(".candle").length / 2, host.scrollLeft, event.clientX - host.getBoundingClientRect().left, chartZoom); const svg = host.querySelector("svg"); const width = Number(svg?.viewBox.baseVal.width) || 0; const x = 58 + index * (width - 170) / Math.max(index ? host.querySelectorAll(".candle").length / 2 - 1 : 1, 1); crosshair.setAttribute("d", `M${x} 28V740`); crosshair.hidden = false; });
  host.addEventListener("pointerleave", () => { const crosshair = host.querySelector(".chart-crosshair"); if (crosshair) crosshair.hidden = true; });
