...
function renderChart(host, item, ticks, intraday, unit, range) {
  const legend = document.getElementById("analysis-chart-legend"); const prices = document.getElementById("analysis-chart-prices"); const axis = document.getElementById("analysis-chart-axis");
  const allBars = barsForUnit(item.bars, ticks, intraday, unit); const bars = rangeBars(allBars, range, unit);
  if (!bars.length) { host.textContent = ["틱", "초", "분", "시"].includes(unit) ? "실시간 체결 데이터가 아직 없습니다. 장중 체결 후 표시됩니다." : "일별 데이터가 없습니다. 다음 동기화 뒤 다시 확인하세요."; legend.replaceChildren(); prices.replaceChildren(); axis.replaceChildren(); return; }
  const scrollRight = Math.max(0, host.scrollWidth - host.clientWidth - host.scrollLeft); const start = Math.max(0, allBars.length - bars.length); const width = chartWidth(bars.length); const nextScrollLeft = chartScrollLeft(bars.length, host.clientWidth, scrollRight); const priceTop = 28; const priceBottom = 310; const volumeTop = 340; const volumeBottom = 415; const rsiTop = 455; const rsiBottom = 535; const left = 58; const right = width - 55;
  ...
  host.innerHTML = `<svg class="holding-chart" style="width:${width}px;height:565px" viewBox="0 0 ${width} 565" role="img" aria-label="${item.name} 가격, 거래량, RSI 차트"><g>${grid}${candles}${purchaseMarks}<path d="${linePath(ma20, x, y)}" class="ma20"/><path d="${linePath(ma60, x, y)}" class="ma60"/><path d="${linePath(ma120, x, y)}" class="ma120"/>${marker(number(item.averagePurchasePrice), "purchase")}${marker(number(item.lastPrice), "current")}</g><g><text x="${left}" y="${volumeTop - 9}" class="chart-label">거래량</text><path d="M${left} ${volumeBottom}H${right}" class="chart-grid"/>${volumes}</g><g><text x="${left}" y="${rsiTop - 9}" class="chart-label">RSI 14</text><path d="M${left} ${rsiY(70)}H${right}M${left} ${rsiY(30)}H${right}" class="rsi-grid"/><path d="${linePath(rsi14, x, rsiY)}" class="rsi"/><text x="${right + 10}" y="${rsiY(70) + 4}" class="chart-axis">70</text><text x="${right + 10}" y="${rsiY(30) + 4}" class="chart-axis">30</text>${labels}</g></svg>`;
  legend.innerHTML = `<span class="ma20">20${maUnit}선</span><span class="ma60">60${maUnit}선</span><span class="ma120">120${maUnit}선</span>`; prices.innerHTML = `<span class="purchase">평균매수가 ${won(item.averagePurchasePrice)}</span><span class="current">현재가 ${won(item.lastPrice)}</span>`; axis.innerHTML = priceAxis.map(({ value, top }) => `<span style="top:${top - 9}px">${Math.round(value).toLocaleString("ko-KR")}</span>`).join("");
