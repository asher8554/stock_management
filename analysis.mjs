...
function renderChart(host, item, ticks, intraday, unit, range) {
  ...
  const scrollRight = Math.max(0, host.scrollWidth - host.clientWidth - host.scrollLeft); const start = Math.max(0, allBars.length - bars.length); const width = chartWidth(bars.length); const nextScrollLeft = chartScrollLeft(bars.length, host.clientWidth, scrollRight); const priceTop = 28; const priceBottom = 290; const volumeTop = 318; const volumeBottom = 378; const rsiTop = 410; const rsiBottom = 470; const macdTop = 510; const macdBottom = 580; const left = 58; const right = width - 112;
