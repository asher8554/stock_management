...
function renderChart(host, item, ticks, intraday, unit, range) {
  ...
  host.innerHTML = `<svg class="holding-chart" style="width:${width}px" viewBox="0 0 ${width} 565" role="img" aria-label="${item.name} 가격, 거래량, RSI 차트"><g>${grid}${candles}<path d="${linePath(ma20, x, y)}" class="ma20"/><path d="${linePath(ma60, x, y)}" class="ma60"/><path d="${linePath(ma120, x, y)}" class="ma120"/>${marker(number(item.averagePurchasePrice), "평균매수가", "purchase")}${marker(number(item.lastPrice), "현재가", "current")}</g><g><text x="${left}" y="${volumeTop - 9}" class="chart-label">거래량</text><path d="M${left} ${volumeBottom}H${right}" class="chart-grid"/>${volumes}</g><g><text x="${left}" y="${rsiTop - 9}" class="chart-label">RSI 14</text><path d="M${left} ${rsiY(70)}H${right}M${left} ${rsiY(30)}H${right}" class="rsi-grid"/><path d="${linePath(rsi14, x, rsiY)}" class="rsi"/><text x="${right + 10}" y="${rsiY(70) + 4}" class="chart-axis">70</text><text x="${right + 10}" y="${rsiY(30) + 4}" class="chart-axis">30</text>${labels}</g><g class="chart-legend"><text x="${left}" y="18">20${maUnit}선</text><text x="${left + 58}" y="18">60${maUnit}선</text><text x="${left + 116}" y="18">120${maUnit}선</text><text x="${left + 195}" y="18">상승</text><text x="${left + 237}" y="18">하락</text></g></svg>`;
