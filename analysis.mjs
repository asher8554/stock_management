...
function renderChart(host, item, unit, range, zoom) {
  ...
  const scaleBars = chartViewportBars(bars, nextScrollLeft, host.clientWidth, zoom); const purchaseEvents = purchaseMarkersForUnit(unit, bars, item.purchases); const visiblePurchasePrices = purchaseEvents.filter(({ index }) => scaleBars.includes(bars[index])).map((purchase) => number(purchase.price)); const lows = scaleBars.map((bar) => bar.low); const highs = scaleBars.map((bar) => bar.high); const min = Math.min(...lows, number(item.averagePurchasePrice), number(item.lastPrice), ...visiblePurchasePrices); const max = Math.max(...highs, number(item.averagePurchasePrice), number(item.lastPrice), ...visiblePurchasePrices); const padding = Math.max((max - min) * 0.08, 1);
...
async function init() {
  ...
  const render = () => {
    ...
    renderChart(host, selected, selectedUnit, selectedRange, chartZoom);
  ...
  host.addEventListener("pointermove", (event) => { const crosshair = host.querySelector(".chart-crosshair"); if (!crosshair) return; const index = chartHoverIndex(host.querySelectorAll(".candle").length / 2, host.scrollLeft, event.clientX - host.getBoundingClientRect().left, chartZoom); const svg = host.querySelector("svg"); const width = Number(svg?.viewBox.baseVal.width) || 0; const x = 58 + index * (width - 170) / Math.max(index ? host.querySelectorAll(".candle").length / 2 - 1 : 1, 1); crosshair.setAttribute("d", `M${x} 28V740`); crosshair.removeAttribute("hidden"); });
  host.addEventListener("pointerleave", () => { const crosshair = host.querySelector(".chart-crosshair"); if (crosshair) crosshair.setAttribute("hidden", ""); });
