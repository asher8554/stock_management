...
async function init() {
  ...
  host.addEventListener("pointermove", (event) => { const crosshair = host.querySelector(".chart-crosshair"); if (!crosshair) return; const pointerX = event.clientX - host.getBoundingClientRect().left; const index = chartHoverIndex(host.querySelectorAll(".candle").length / 2, host.scrollLeft, pointerX, chartZoom); const svg = host.querySelector("svg"); const width = Number(svg?.viewBox.baseVal.width) || 0; const x = 58 + index * (width - 170) / Math.max(index ? host.querySelectorAll(".candle").length / 2 - 1 : 1, 1); crosshair.setAttribute("d", `M${x} 28V740`); crosshair.removeAttribute("hidden"); const bar = chartBars[index]; const tooltip = document.getElementById("analysis-chart-tooltip"); if (!bar || !tooltip) return; tooltip.textContent = `${timeLabel(bar.time, selectedUnit)} · 종가 ${money(bar.close, chartCurrency)}`; tooltip.removeAttribute("hidden"); tooltip.style.left = `${Math.max(8, Math.min(host.clientWidth - tooltip.offsetWidth - 8, pointerX + 12))}px`; tooltip.style.top = `${Math.max(8, Math.min(host.clientHeight - tooltip.offsetHeight - 8, event.clientY - host.getBoundingClientRect().top + 12))}px`; });
