...
function renderChart(host, item, ticks, intraday, unit, range) {
  ...
  legend.innerHTML = `<span class="ma20">20${maUnit}선</span><span class="ma60">60${maUnit}선</span><span class="ma120">120${maUnit}선</span><span class="purchase">평균 ${won(item.averagePurchasePrice)}</span><span class="current">현재 ${won(item.lastPrice)}</span>`; axis.innerHTML = priceAxis.map(({ value, top }) => `<span style="top:${top - 9}px">${Math.round(value).toLocaleString("ko-KR")}</span>`).join("");
