...
function renderChart(host, item, ticks, intraday, unit, range, zoom) {
...
async function init() {
  ...
  let items = []; let realtime = {}; let intraday = {}; let chartZoom = Math.min(3, Math.max(.35, Number(localStorage.getItem(ANALYSIS_ZOOM_STORAGE_KEY)) || 1)); let rendering = false; let renderedScrollLeft = -1; let scrollFrame;
  const render = () => {
    ...
    renderChart(host, selected, realtime[selected.symbol] || [], intraday[selected.symbol] || [], selectedUnit, selectedRange, chartZoom);
